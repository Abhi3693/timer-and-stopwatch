        ...
        <div className='stopwatch-display'>
          <span className='hours'>
            {this.state.milliseconds
              ? '0' +
                Math.floor((this.state.milliseconds / 360000) % 60) +
                ' : '
              : ' 00 :'}
          </span>
          <span className='minutes'>
            {this.state.milliseconds
              ? '0' + Math.floor((this.state.milliseconds / 6000) % 60) + ' : '
              : ' 00 :'}
          </span>
          <span className='seconds'>
            {this.state.milliseconds
              ? Math.floor((this.state.milliseconds / 100) % 60) + ' : '
              : ' 00 :'}
          </span>
          <span className='milliseconds'>
            {this.state.milliseconds
              ? Math.floor(this.state.milliseconds % 100)
              : ' 00'}
          </span>
        </div>
        {!this.state.isStarted ? (
          <button onClick={this.handleStart} className='btn'>
            Start
          </button>
        ) : !this.state.isStopped ? (
          <button onClick={this.handleStop} className='btn'>
            Stop
          </button>
        ) : (
          <div>
            <button onClick={this.handleResume} className='btn'>
              Resume
            </button>
